...
const ROLE_RULES = [
    ...
    { type: 'dessert', match: ['cake', 'cookie', 'pie', 'brownie', 'ice cream', 'truffle', 'pudding'] },
    { type: 'base', match: ['rice', 'quinoa', 'dough', 'stock', 'dressing', 'sauce', 'vinaigrette'] },
    { type: 'side', match: ['salad', 'vegetable', 'beans', 'sprouts', 'broccoli', 'corn', 'potatoes', 'carrots', 'asparagus'] },
    { type: 'main', match: ['chicken', 'beef', 'pork', 'lamb', 'shrimp', 'fish', 'salmon', 'trout', 'burger', 'pasta', 'lasagna', 'stew', 'soup', 'chili', 'tacos', 'enchiladas'] }
];

const VIBE_RULES = [
    { type: 'holiday', match: ['thanksgiving', 'turkey', 'stuffing', 'cranberry', 'rosh hashanah', 'brisket'] },
    { type: 'project', match: ['lasagna', 'risotto', 'paella', 'dumpling', 'pie', 'cake', 'wonton'] },
    { type: 'comfort', match: ['mac and cheese', 'alfredo', 'marsala', 'parmesan', 'meatloaf', 'braise', 'stew', 'soup', 'chili'] },
...
function refine() {
    ...
    files.forEach(file => {
        ...
        if (data.role === 'main' && !content.includes('## Serving Suggestions') && !content.includes('## Pairs With')) {
