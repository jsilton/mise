---
title: Chicken Lo Mein with Shiitake and Bok Choy
origin: Chinese-American home cooking with Cantonese roots
difficulty: intermediate
role: main
vibe: comfort
prepTime: 30 min
cookTime: 35 min
totalTime: 1 hr 5 min
servings: '5'
cuisines:
  - Chinese-American
  - Cantonese-inspired
cookingMethods:
  - stir-fry
  - boil
  - sear
dietary:
  - dairy-free
occasions:
  - weeknight
  - family-meal
  - kid-friendly
  - meal-prep
  - entertaining
flavorProfile:
  - savory
  - umami
  - earthy
  - aromatic
seasons:
  - year-round
nutritionalDensity: hearty
leftovers: good
advancePrep:
  - make-ahead-sauce
  - prep-vegetables
  - components-ahead
equipment:
  - 14-inch-wok-or-12-inch-deep-skillet
  - large-noodle-pot
  - colander
  - instant-read-thermometer
  - two-serving-platters
usesBase: []
pairsWith:
  - smashed-cucumber-salad
  - steamed-edamame
ingredients:
  - '--- Noodles and chicken ---'
  - '450 g (1 lb) fresh raw lo mein egg noodles, not dried noodles or pre-cooked noodles'
  - '450 g boneless skinless chicken thighs, sliced across the grain about 1/4 inch thick'
  - 2 tsp cornstarch
  - '1 tsp neutral oil, for the chicken'
  - '1 tsp light soy sauce, for the chicken'
  - '1/8 tsp ground white pepper, for the chicken'
  - '--- Sauce, divided between two finishing batches ---'
  - 2 tbsp light soy sauce
  - 2 tsp dark soy sauce
  - 1 tbsp oyster sauce
  - 1/4 cup (60 ml) low-sodium chicken broth or water
  - 1 tsp sugar
  - 1/8 tsp ground white pepper
  - '--- Pan ingredients ---'
  - '3 tbsp neutral oil, divided'
  - '3 cloves garlic, minced'
  - 1 tbsp minced fresh ginger
  - '4 scallions, whites thinly sliced and greens cut into 1-inch lengths'
  - '225 g fresh shiitake caps, tough stems removed, sliced 1/4 inch thick'
  - '340 g baby bok choy, washed and thoroughly dried, stems and leaves separated'
  - '150 g mung bean sprouts, rinsed and drained'
  - 1 tbsp Shaoxing wine
  - '2 tsp toasted sesame oil, divided between finishing batches'
  - '1 tsp Chinkiang black vinegar, optional, divided between finishing batches'
  - '1/4 cup (60 ml) hot water, only as needed to loosen noodles'
  - '15 g prepared fried shallots, optional garnish'
description: >-
  Boiled egg noodles folded through chicken, shiitake and bok choy in two manageable pan batches,
  with a separate shrimp variation.
learning:
  focus: Prepare the noodle type correctly and finish in two pan-sized batches
  outcome: >-
    Separate, lightly sauced noodles with tender chicken, browned mushroom patches and cooked
    vegetables that retain shape.
  techniques:
    - browning
    - starch
    - temperature
    - seasoning
  before:
    - >-
      The 450 g noodle weight is for fresh raw egg noodles. Dried and pre-cooked packages contain
      different amounts of water, so equal weights are not interchangeable.
    - >-
      Divide noodles, vegetables, sauce and finishing oils into two portions before the final
      stir-fry. A five-person heap does not need to fit the pan at once.
  checkpoints:
    - step: 3
      cue: 'A tasted noodle is cooked through and springy, without a chalky center or a mushy surface.'
      why: >-
        Thickness and noodle formulation matter more than a universal number of seconds shaved from
        the package time.
    - step: 4
      cue: Chicken has some browned surfaces and the thickest pieces in each batch reach 165°F / 74°C.
      why: >-
        The final toss should distribute finished chicken, not depend on residual heat to finish an
        undercooked center.
    - step: 5
      cue: Mushroom surfaces have brown patches and the bok choy stems are tender with some resistance.
      why: >-
        Stems, leaves and mushrooms do not soften at the same rate; separate additions preserve
        their different textures.
    - step: 6
      cue: >-
        Sauce coats the strands, sprouts are thoroughly hot and cooked, and liquid does not pool
        beneath the noodles.
      why: >-
        A controlled pan load lets the food heat evenly. Raw-crisp sprouts added off heat are a
        different handling choice.
  troubleshooting:
    - problem: Noodles clump into a sticky mass
      cause: 'They were overcooked, poorly drained or left compressed for too long.'
      fix: >-
        Loosen gently with a little of the measured hot water and fold rather than chop with the
        spatula. Next time taste early, drain thoroughly and divide before stir-frying.
    - problem: The dish tastes very salty
      cause: 'Soy and oyster sauce brands vary, or extra sauce was added to compensate for crowding.'
      fix: >-
        Use the measured starting sauce and taste each finished batch. Additional unsalted noodles
        or vegetables can dilute salt; vinegar does not remove it.
  substitutions:
    - ingredient: 450 g chicken
      alternative: 450 g thawed raw peeled and deveined shrimp
      effect: >-
        Omit the chicken soy marinade and use its cornstarch, teaspoon oil and white pepper as a
        light coating. Cook in two batches until the thickest flesh is pearly and opaque throughout,
        then return only long enough to heat with the noodles. Do not remove at a guessed 80 percent
        and rely on off-heat carryover.
    - ingredient: 3 tbsp neutral pan oil
      alternative: 'The same measured amount of strained scallion-shallot oil, already prepared'
      effect: >-
        Adds an allium aroma; keep solids out of the hot pan and reserve them for garnish. Its
        separate preparation time and storage rules apply.
    - ingredient: 450 g fresh raw noodles
      alternative: '300 g dried wheat egg noodles, cooked separately until springy'
      effect: >-
        This is an approximate starting quantity, not a universal package conversion. Weighing the
        original raw noodles remains the most reproducible version; divide any alternative into
        manageable pan loads.
  timing: >-
    Allow about 65 minutes for one cook: slicing, washing and drying, noodle preparation and
    separate chicken/vegetable/finishing batches. Chicken coating rests in the refrigerator while
    other preparation happens; there is no mandatory fifteen-minute maximum.
  storage: >-
    Refrigerate in shallow containers within 2 hours (1 hour above 90°F / 32°C), at 40°F / 4°C or
    below. Use within 3–4 days; reheat leftovers to 165°F / 74°C. Noodles soften on reheating.
    Reheat covered in the microwave or gently in a skillet with a small splash of water, stirring
    and checking several places. Neither method guarantees freshly cooked texture.
  sources:
    - title: Bill Leung — Preparing raw and cooked lo mein noodles
      url: 'https://thewoksoflife.com/lo-mein-noodles-how-to-cook/'
    - title: Made With Lau — One Cantonese lo mein method
      url: 'https://www.madewithlau.com/recipes/vegetable-lo-mein'
    - title: FoodSafety.gov — Safe cooking temperatures
      url: 'https://www.foodsafety.gov/food-safety-charts/safe-minimum-internal-temperatures'
    - title: 'FoodSafety.gov — Clean, separate, cook, chill'
      url: 'https://www.foodsafety.gov/keep-food-safe/4-steps-to-food-safety'
    - title: USDA FSIS — Leftovers and food safety
      url: >-
        https://www.fsis.usda.gov/food-safety/safe-food-handling-and-preparation/food-safety-basics/leftovers-and-food-safety
    - title: FDA — Selecting and serving produce safely
      url: 'https://www.fda.gov/food/buy-store-serve-safe-food/selecting-and-serving-produce-safely'
    - title: American Chemical Society — Chemistry of grilling
      url: 'https://www.acs.org/pressroom/reactions/library/grilling.html'
  review:
    status: editorial-review
    date: '2026-09-06'
---

## Chef's Note

This is a home-pan lo mein with a light coating of sauce. Cantonese noodle preparations vary; one recipe does not define the whole category. Dark soy contributes flavor and salt as well as color. Toasted sesame oil goes in at the finish for a clear aroma, without claiming that all of its flavor disappears at a single temperature.

## Directions

1. **Coat the chicken and prepare the sauce:** Toss chicken with its cornstarch, 1 tsp oil, 1 tsp light soy and white pepper; refrigerate while preparing everything else. Whisk the six sauce ingredients together and divide into two equal portions.
2. **Set up the vegetables:** Keep mushroom slices, bok choy stems, leaves, sprouts and scallion greens separate. Cut stems into 1/2-inch pieces and large leaves into manageable pieces. Mix garlic, ginger and scallion whites. Divide every vegetable and aromatic group, the Shaoxing wine, sesame oil and optional vinegar into two portions for the final rounds.
3. **Prepare noodles for this product:** Bring a large pot of unsalted water to a full boil. Add fresh raw noodles, separate gently and cook according to their package range, tasting early until cooked through but springy. Rinse briefly with cold water, drain thoroughly and toss with 1 tsp of the measured pan oil. Divide into two loose piles and proceed promptly; start the pan work as soon as the noodles are ready.
4. **Cook the chicken fully in two batches:** Heat the wok or deep skillet over medium-high to high as appropriate for the pan, then add 2 tsp pan oil. Spread half the chicken in a layer. Leave about 1 minute for color, then turn and stir for another 2–3 minutes or until the thickest slices reach 165°F / 74°C. Transfer to a clean plate. Repeat with another 2 tsp oil and the remaining chicken. Keep the cooked chicken divided for the two finishing rounds.
5. **Cook the first vegetable portion:** Add 2 tsp of the remaining oil. Cook half the mushrooms 3–4 minutes, stirring occasionally, until some surfaces brown. Add half the aromatics for 20–30 seconds, then half the wine and bok choy stems. Stir-fry about 2 minutes, adding a little measured hot water if needed for the stems to soften. Add half the leaves, sprouts and scallion greens and cook another 2–3 minutes, stirring, until thoroughly hot and cooked rather than raw-crisp.
6. **Finish the first noodle batch:** Add half the noodles and one portion of sauce. Fold from the bottom for about 1–2 minutes, using a little measured hot water only if needed to separate strands. Add half the cooked chicken and toss over the heat until everything is hot and evenly coated. Turn off the heat and fold in half the sesame oil and optional vinegar. Transfer to a warm serving platter and cover loosely for the short second round.
7. **Repeat and serve:** Use the last 2 tsp pan oil and the remaining vegetables, aromatics, wine, noodles, sauce and chicken to repeat steps 5–6. Finish with the remaining sesame oil and optional vinegar. Divide both batches among five bowls and add the optional measured fried shallots. Serve promptly.

## Scaling and variations

The pan oil is fully allocated: 1 tsp on noodles, 2 tsp for each chicken batch and 2 tsp for each vegetable round, totaling 3 tbsp. The chicken coating has its own separate teaspoon. If scaling up, add complete cooking rounds; multiplying ingredients does not make the pan larger. At half batch, a single finishing round is appropriate for the same pan, while the noodle type still determines boiling time.

For the optional aromatic oil, use [Scallion-Shallot Oil](/mise/recipes/scallion-shallot-oil) already made and stored as directed. Neutral oil is the complete base method. For a cool accompaniment, see [Smashed Cucumber Salad](/mise/recipes/smashed-cucumber-salad).
