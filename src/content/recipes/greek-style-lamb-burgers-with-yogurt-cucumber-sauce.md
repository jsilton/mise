---
title: Greek-Style Lamb Burgers with Mint Yogurt
difficulty: easy
cookingMethods:
  - pan-fry
  - no-cook
occasions:
  - entertaining
flavorProfile:
  - acidic
  - rich
  - herbaceous
cuisines:
  - Greek-inspired
role: main
vibe: comfort
prepTime: 25 min
cookTime: 30 min
totalTime: 1 hr
advancePrep: []
servings: 4 filled pitas
pairsWith:
  - greek-village-salad
  - lemon-herb-couscous
ingredients:
  - '--- Lamb patties ---'
  - '680 g ground lamb, preferably about 80–85% lean'
  - '60 g yellow onion, very finely minced'
  - '1 clove garlic, finely grated, for the patties'
  - '3 tbsp finely chopped fresh mint, for the patties'
  - 3 tbsp finely chopped fresh parsley
  - '1 tsp Diamond Crystal kosher salt or 1/2 tsp fine salt, for the patties'
  - '1/2 tsp ground black pepper, for the patties'
  - '2 tsp olive oil, for the skillet, divided'
  - '--- Cucumber-mint yogurt ---'
  - 1 cup (240 g) plain full-fat Greek yogurt
  - '100 g English cucumber, finely diced and patted dry'
  - '1 tbsp finely chopped fresh mint, for the sauce'
  - '1 tbsp extra-virgin olive oil, for the sauce'
  - '1 small clove garlic, finely grated, for the sauce'
  - '1 tsp fresh lemon juice, plus 1 tsp more only if needed'
  - '1/4 tsp Diamond Crystal kosher salt or 1/8 tsp fine salt, for the sauce'
  - '1/8 tsp ground black pepper, for the sauce'
  - '--- To serve ---'
  - >-
    4 pita breads, about 6–7 inches across; warm using [Warm, Soft Pita
    Bread](/recipes/warm-pita-bread)
  - '40 g crumbled feta, optional'
  - '200 g ripe tomato, sliced, optional'
  - '40 g mixed greens, washed and dried, optional'
origin: Greek-inspired home cooking
seasons:
  - year-round
nutritionalDensity: moderate
leftovers: good
description: >-
  Herbed lamb patties in warm pita with a cool, thick cucumber-mint yogurt sauce
  and measured optional toppings.
equipment:
  - 12-inch-heavy-skillet
  - instant-read-thermometer
  - oven
  - rimmed-baking-sheet
usesBase:
  - warm-pita-bread
learning:
  focus: 'Build a browned lamb patty without compacting it, then measure its center'
  outcome: >-
    A browned, herb-speckled patty that holds together without a springy sausage
    texture, wrapped in soft bread with cool spoonable sauce.
  techniques:
    - browning
    - temperature
    - seasoning
  before:
    - >-
      Keep lamb cold while chopping the vegetables and making the sauce. Use
      separate utensils and plates for raw meat and ready-to-eat toppings.
    - >-
      The default uses two patties at a time in a 12-inch skillet. Heat the oven
      to 350°F / 175°C for warming the pita during the second batch; the first
      patties will wait briefly on a clean plate.
  checkpoints:
    - step: 2
      cue: >-
        Herbs and fine onion are evenly scattered through meat that still has a
        loose grain.
      why: >-
        Mixing helps the meat bind, but prolonged kneading, especially with
        salt, extracts more sticky proteins and gives a firmer, sausage-like
        bite.
    - step: 3
      cue: >-
        The underside has a brown crust and releases under a broad spatula; the
        center of each patty reaches 160°F / 71°C.
      why: >-
        Surface browning and ground-meat doneness are separate checks. Color or
        a minutes-per-side rule cannot verify the center.
    - step: 5
      cue: The sauce forms a soft mound and the pita folds without breaking.
      why: >-
        Thick yogurt limits free liquid, and bread warmed just before filling is
        more flexible. Yogurt and vegetables still soften bread if assembled far
        ahead.
  troubleshooting:
    - problem: Patty breaks when flipped
      cause: >-
        It was moved before a crust formed, the onion pieces were coarse, or the
        mixture was barely joined.
      fix: >-
        Let the next patty set longer, slide a wide spatula fully underneath,
        and turn once it releases. Gather broken cooked pieces into the pita; do
        not press them repeatedly against the pan.
    - problem: Outside is dark before the center is done
      cause: The pan is too hot for the patty thickness.
      fix: >-
        Lower the heat and turn periodically until the thermometer reaches 160°F
        / 71°C. Shape the remaining patties to an even thickness; do not rely on
        resting to finish an undercooked center.
    - problem: Sauce becomes watery
      cause: Thin yogurt or wet diced cucumber supplied excess free liquid.
      fix: >-
        Spoon off obvious liquid and stir gently. Use thick strained yogurt and
        dry cucumber next time; grated cucumber is also usable if squeezed well.
  substitutions:
    - ingredient: Ground lamb
      alternative: '680 g ground beef, about 80–85% lean'
      effect: >-
        Less lamb flavor; retain the 160°F / 71°C ground-meat endpoint and
        measure each patty.
    - ingredient: Mint yogurt sauce
      alternative: About 1 1/4 cups prepared Tzatziki Sauce
      effect: >-
        Use as a replacement, not an additional sauce. The linked standalone
        tzatziki uses drained grated cucumber and dill and makes more than is
        needed here.
  timing: >-
    About sixty minutes for sauce, chopping, shaping, two 10–14-minute skillet
    batches, bread warming during the second batch and assembly. Sauce chills
    while the meat is prepared; a separate mandatory thirty-minute wait is
    unnecessary. A grill or griddle that holds all four patties can shorten
    elapsed time, but endpoints still govern.
  storage: >-
    Refrigerate cooked patties and sauce separately within 2 hours (1 hour above
    90°F / 32°C), at 40°F / 4°C or below. Use within 3–4 days; reheat patties to
    165°F / 74°C. Keep bread unfilled until serving. Prepared raw patties follow
    the meat’s original use-by/storage limit.
  sources:
    - title: >-
        American Meat Science Association — Burger and meatball shape and
        texture
      url: >-
        https://themeatlocker.org/wp-content/uploads/sites/2/2024/07/AMSA-2024-MeatLocker-Improve-Burger-Shapes.pdf
    - title: FoodSafety.gov — Safe minimum internal temperatures
      url: >-
        https://www.foodsafety.gov/food-safety-charts/safe-minimum-internal-temperatures
    - title: 'FoodSafety.gov — Clean, separate, cook, chill'
      url: 'https://www.foodsafety.gov/keep-food-safe/4-steps-to-food-safety'
    - title: USDA FSIS — Leftovers and food safety
      url: >-
        https://www.fsis.usda.gov/food-safety/safe-food-handling-and-preparation/food-safety-basics/leftovers-and-food-safety
  review:
    status: editorial-review
    date: '2026-09-06'
---

## Chef's Note

Mint and parsley lighten the impression of rich lamb, while thick yogurt adds a cool tang. Fine onion disperses through the patty; large wet pieces create weak spots. Fold the mixture only until it holds together. Shape affects how the burger fits the pita, but an oval is not a special guarantee of rendered fat or crispness.

## Directions

1. **Make the sauce and prepare toppings:** Stir yogurt, the dried diced cucumber, sauce mint, sauce oil, sauce garlic, the initial teaspoon of lemon juice, sauce salt and sauce pepper. Refrigerate while preparing the meat. Slice any optional tomato and wash/dry greens before handling raw lamb. Heat the oven to 350°F / 175°C for the bread.
2. **Mix and shape:** Combine cold lamb with onion, patty garlic, the three tablespoons each of mint and parsley, patty salt and patty pepper. Fold gently just until evenly mixed and cohesive. Divide into four equal patties, each using about 170 g meat plus its share of aromatics. Shape into broad rounds or ovals about 3/4 inch thick. Keep the second pair refrigerated while the first cooks.
3. **Cook the first pair:** Heat a 12-inch heavy skillet over medium-high heat. Add 1 tsp of the skillet oil and two patties with space between them. Cook about 4–5 minutes on the first side until well browned, then turn with a broad spatula. Reduce to medium as needed and cook another 4–5 minutes, then check from the side into each center. Continue cooking and turning as needed until each reaches 160°F / 71°C; allow roughly 10–14 minutes per batch. Do not press out the juices. Transfer to a clean plate and cover loosely.
4. **Cook the second pair and warm bread:** Carefully spoon excessive rendered fat into a heatproof container, leaving a thin film in the pan; add the remaining 1 tsp skillet oil. Cook the second pair with the same temperature check. During this batch, warm the four pitas as a foil-wrapped stack using the [oven method](/mise/recipes/warm-pita-bread#oven-alternative), without the optional finishing oil or salt. Check the center bread and keep the stack wrapped. The first patties soften and cool slightly during this brief wait; a large griddle avoids the wait if available.
5. **Balance and assemble:** Stir and taste the chilled sauce, adding the reserved teaspoon of lemon only if needed. Start with 2–3 tbsp inside each pita and offer the remaining sauce alongside. Add one cooked patty to each and divide any optional feta, tomato and greens among them. Fold or serve open-faced if the bread is small. Serve promptly.

## Grill option

Use clean grates and a medium-high cooking zone with a cooler area available. Use the same two teaspoons of cooking oil to lightly brush the patties instead of adding it to a skillet. Turn when a crust forms; move away from flare-ups caused by dripping fat. Cook every patty to 160°F / 71°C measured at its center. Warm the bread separately and assemble as above.

## Sauce alternative

If [Tzatziki Sauce](/mise/recipes/tzatziki-sauce) is already prepared, use about 1 1/4 cups in place of the entire cucumber-mint sauce. Do not make both. Keep its original preparation date when planning storage.
